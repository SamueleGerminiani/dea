...
do
    ...
    vsim work.sobel_tb -c -voptargs="+acc" -do "run -all; quit" 
    mv IO/out/512x512sobel_out_nbits.txt imgs/planeBR_cluster/cluster$cluster.txt
done
...
rm ssimBR_cluster.csv
    ...
    for cluster in "${!cToMock[@]}"
    do
        python3 scripts/sobel_IO_to_jpeg.py imgs/planeBR_cluster/"cluster$cluster.txt" imgs/planeBR_cluster/"cluster$cluster.jpeg"
    done

##get ssim
for cluster in "${!cToMock[@]}"
do
    python3 scripts/calc_SSIM.py imgs/planeBR_cluster/golden.jpeg imgs/planeBR_cluster/"cluster$cluster.jpeg"
    ssim=$(head -n 1 ssim_out.txt)
    echo "c$cluster,$ssim" >> ssimBR_cluster.csv
done
