...
ssimCluster="../simulator/ssim/ssimSR_cluster.csv"
ssimClusterRandom="../simulator/ssim/ssimSR_cluster_random.csv"
...
if [ "$1" = -p ]; then
    rm plotClusterSR.csv
    echo "x,size,ssimCluster,ssimClusterRandom" >> "plotClusterSR.csv"
    for ((k=0;k<i;k++)); do
        echo "$k,${xTSize[$k]},${xTcs[$k]},${xTrcs[$k]}" >> "plotClusterSR.csv"
    done
else
    rm plotClusterSR.csv
fi
