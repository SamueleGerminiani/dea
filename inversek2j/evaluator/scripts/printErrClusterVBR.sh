...
errCluster="../simulator/err/errVBR_cluster.csv"
errClusterRandom="../simulator/err/errVBR_cluster_random.csv"
...
if [ "$1" = -p ]; then
    rm plotClusterVBR.csv
    echo "x,size,errCluster,errClusterRandom" >> "plotClusterVBR.csv"
    for ((k=0;k<i;k++)); do
        echo "$k,${xTSize[$k]},${xTcs[$k]},${xTrcs[$k]}" >> "plotClusterVBR.csv"
    done
else
    rm plotClusterVBR.csv
fi
